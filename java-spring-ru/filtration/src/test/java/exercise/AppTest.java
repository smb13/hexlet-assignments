...
public class AppTest {
    ...
    void testFilterByFirstAndLastNameAndEmailAndProfessionAndGender() throws Exception {
        MockHttpServletResponse response = mockMvc
                .perform(get("/users?firstName=t&lastName=a&email=mysql&profession=eng&gender=Female"))
                .andReturn()
                .getResponse();
        ...
        User actualUser = actualUsers.get(0);
        assertThat(actualUsers.size()).isEqualTo(1);
        assertThat(actualUser.getFirstName()).isEqualTo("Retha");
        assertThat(actualUser.getLastName()).isEqualTo("Astupenas");
    }
    // END
}
